...
namespace WTB.SDK.Config
{
    ...
    internal class INIFile
    {
        ...
        internal INIFile(string p_FilePath)
        {
            ...
            if (File.Exists(p_FilePath))
            {
                ...
                {
                    Logger.log?.Error("[SDK.Config][IniFile] Failed to read configuration");
        ...
        internal string GetSetting(string p_Section, string p_Key)
        {
            lock (m_Values)
            {
        ...
        internal void SetSetting(string p_Section, string p_Key, string p_Value)
        {
            lock (m_Values)
            {
                SortedDictionary<string, string> l_Section = null;
            ...
            HMMainThreadDispatcher.instance.Enqueue(() =>
            {
                ...
                {
                    Logger.log?.Error("[SDK.Config][IniFile.SetSetting] failed to save configuration");
                    Logger.log?.Error(l_Exception);
                }
            });
        }
        ...
        internal void SaveSettings()
        {
            lock (m_Values)
            {
                var l_StrToSave = new StringBuilder();
                foreach (var l_Section in m_Values)
                {
                    l_StrToSave.Append("[" + l_Section.Key + "]\r\n");
                    foreach (var l_Value in l_Section.Value)
                    {
                        if (l_Value.Value == null)
                            continue;

                        l_StrToSave.Append(l_Value.Key + " = " + l_Value.Value + "\r\n");
                    }
                    l_StrToSave.Append("\r\n");
                }
                ...
                try
                {
                    ...
                    l_TextWritter.Write(l_StrToSave.ToString());
                    l_TextWritter.Close();
                }
                catch (Exception l_Exception)
                {
