...
namespace WTB.SDK.Network
{
    ...
    internal sealed class APIClient
    {
        ...
        /// </summary>
        internal int MaxRetry = 5;
        /// <summary>
        /// Delay between each retry
        /// </summary>
        internal int RetryInterval = 5 * 1000;
        ...
        internal APIClient(string p_BaseAddress, TimeSpan p_TimeOut, bool p_KeepAlive = true)
        {
        ...
        internal async Task<APIResponse> GetAsync(string p_URL, CancellationToken p_Token, bool p_DontRetry = false)
        {
            ...
            for (int l_Retry = 0; l_Retry < MaxRetry; l_Retry++)
            {
                ...
                if (l_Reply != null)
                    Logger.log.Error($"[SDK.Network][APIClient.GetAsync] Request failed with code {l_Reply.StatusCode}:\"{l_Reply.ReasonPhrase}\", next try in 5 seconds...");
                else
                    Logger.log.Error($"[SDK.Network][APIClient.GetAsync] Request failed, next try in 5 seconds...");
        ...
        /// <param name="p_Content">Request content</param>
        /// <returns>HTTPResponse</returns>
        internal async Task<APIResponse> PostAsync(HttpContent p_Content, CancellationToken p_Token, bool p_DontRetry = false)
        {
            p_Token.ThrowIfCancellationRequested();
            ...
            for (int l_Retry = 0; l_Retry < MaxRetry; l_Retry++)
            {
                ...
                try
                {
                    l_Reply = await m_Client.PostAsync("?_bsp_sdk_seed_=" + SDK.Misc.Time.UnixTimeNow(), p_Content, p_Token);
                ...
                if (l_Reply != null)
                    Logger.log.Error($"[SDK.Network][APIClient.PostAsync] Request failed with code {l_Reply.StatusCode}:\"{l_Reply.ReasonPhrase}\", next try in 5 seconds...");
                else
                    Logger.log.Error($"[SDK.Network][APIClient.PostAsync] Request failed, next try in 5 seconds...");
